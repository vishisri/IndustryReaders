...
(function () {
  ...
  // Emerging players: Auto's shape nests exposed_entities[]/mechanism under
  // value_chain_impact; A&D's source data instead puts capability/
  // exposed_entities(a single string)/mechanism flat on the player object.
  // Normalize both into Auto's shape so app.js only ever renders one form.
  const emergingPlayers = ((E && E.emerging_players) || []).map(p => {
    if (p.value_chain_impact) return p;
    return {
      ...p,
      core_differentiation: p.core_differentiation || p.capability,
      value_chain_impact: {
